  ...
  return (
    <header
      style={{
        ...
        // NOTE: do NOT set `backdrop-filter` here. The header background
        // is already fully opaque, so backdrop-filter has no visible
        // effect anyway, AND any non-`none` backdrop-filter creates a
        // containing block for `position: fixed` descendants. That
        // breaks the SDK PasswordlessAuthWidget's modal (its
        // `.fs-auth-modal-backdrop` is `position: fixed; inset: 0`),
        // pinning the modal to the header instead of the viewport.
      ...
      >
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: isMobile ? 8 : 14, minWidth: 0 }}>
          ...
          {isAuthenticated && user ? (
            ...
            <div
              className="conviction-navbar-auth"
              data-testid="navbar-sign-in"
              data-compact={isMobile || isNarrow ? 'true' : 'false'}
            >
              {/*
                We render the SDK's PasswordlessAuthWidget directly
                here, not wrapped in our own modal. The widget already
                manages its own modal (`.fs-auth-modal-backdrop`) when
                its trigger button is clicked; wrapping it caused a
                modal-inside-a-modal stacking bug where the SDK's
                username form overlaid our editorial chrome.

                Global CSS in `index.css` under
                `.conviction-navbar-auth` hides the SDK's redundant
                "Sign In to Trade" hint span and restyles the trigger
                as our ember pill so it visually matches the rest of
                the nav.
              */}
              <PasswordlessAuthWidget
                onLogin={(loggedInUser) => rememberUsername(loggedInUser.username)}
              />
            </div>
          )}
        </div>
