...
*Last updated: 2026-05-18 (receipt desktop polaroid fix: the Receipt page grid now reserves a fixed 380px artifact column with `minmax(0, 1fr) minmax(0, 380px)` so long title/content in the left column wraps instead of pushing the polaroid past the viewport. Chromium verification at 1024x768 on `/r/26/m26_pos_2212` confirms no horizontal document overflow, a fully visible 380 x 570 SVG, a square 348 x 348 photo, and visible caption text. Production Vercel was rebuilt, public SSO protection disabled, and `https://conviction-receipts.vercel.app/` now points at the verified deployment. Conviction suite is now 491 tests across 37 files.) Update this file whenever behavior changes.*
