...
*Last updated: 2026-05-11 (rarity tier system + Step 4 co-location fix: 6 tiers from Common to Mythic; on Step 4 the consensus chart and the live polaroid preview are now always visible together — desktop keeps the sticky right-aside (now with `max-height: calc(100vh - 104px)` + internal scroll so nothing clips on short screens); mobile adds a compact 200px-wide inline preview right above the chart so chart and polaroid co-exist in a 390x844 viewport; Playwright verification (`verify-step4-pair.mjs`) confirms both visible together on 1280x900 desktop AND 390x844 mobile; total 217 Conviction tests + 18+ browser checks). Update this file whenever behavior changes.*
