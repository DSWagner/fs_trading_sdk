...
- `npx vitest run tests/conviction` (Conviction suite): **500 passed across 37 files, including the 5 live smoke tests**. The SDK now accepts the live engine's renamed `state_vector` coefficient field in addition to `alpha_vector`, so market discovery and single-market queries no longer blank the app during refresh.
- Desktop receipt layout: fixed the `/r/:marketId/:positionId` two-column grid so long title and metadata content cannot force the polaroid column off-screen. The desktop grid now uses `minmax(0, 1fr) minmax(0, 380px)`, preserving the full 380 x 570 SVG artifact at 1024px desktop widths. Regression coverage lives in `tests/conviction/receipt-fallback.test.tsx`, and a real Chromium check against `/r/60/m60_pos_2208` confirms `document.scrollWidth === viewport width`, the polaroid frame sits fully inside the viewport, the photo clip is 348 x 348, all SVG definition IDs are unique across repeated receipt/comparison polaroids, and the caption text is present.
...
- **OG / Twitter meta tags + favicon + share card.** `demo-app/index.html` now ships full `og:*` and `twitter:*` tags pointing at the absolute `https://conviction-receipts.vercel.app/og-card.png` URL and a brand favicon at `/favicon.svg`. The editable source artwork remains `demo-app/public/og-card.svg`, and the crawler-facing PNG lives beside it at `demo-app/public/og-card.png` so X and other platforms that reject SVG previews still get a 1200x630 `summary_large_image` card.
...
- ✅ Real test suite (500 conviction-specific tests across 37 files: pure functions, render, achievement math, error-boundary class behaviour, share-kit fallbacks, replay animation, comparison-pair moments analysis + render, polaroid `predictionLabel` regression, polaroid back-hill PDF normalization, polaroid receipt-frame sizing contract, desktop receipt-grid overflow regression, mobile receipt-width regression, duplicate SVG-def ID regression, matching main/comparison Polaroid silhouette regression, exact consensus-curve crowd Polaroid regression, comparison-pair no-clip regression, profile section ordering, receipt share-panel placement, live data integrations, plus the four 2026-05-14 flagships still in the build: streak math + halo render, Receipt-for-Receipt challenge plumbing + button visibility, calibration score + leaderboard aggregation + render, Ed25519 receipt-NFT sign/verify roundtrip + tamper detection + verify-badge render. Plus live API smoke. The Convex-Hull-Frontier widget that briefly shipped alongside these flagships was removed on 2026-05-17 after user feedback; its 14 dedicated tests were deleted at the same time.)
...
- **Dynamic per-receipt OG cards.** The static `og-card.png` now gives X and other social platforms a reliable project-level thumbnail. A richer future version could generate per-receipt images server-side, but that would require infrastructure beyond the zero-backend competition build.
...
**Total Conviction tests: 500 across 37 files, all passing including live smoke** (rarity + polaroid + storage + hash + markdown + editorial + bet-journey + polaroid-render + polaroid-rarity + polaroid-aurora + live-engine + cashout + cashed-out-stamp + live-portfolio + the-wire + drift-sparkline + receipt-fallback + comparison-pair + achievements + achievements-strip + error-boundary + share-kit + streak + streak-halo-render + challenge + challenge-button-render + calibration + leaderboard-render + receipt-nft + verified-receipt-badge-render + demo-galleries + develop-demo-calibration + cashout-storage + live-consensus-card). Plus 15+ Playwright real-browser checks across three verification scripts.
...
*Last updated: 2026-05-18 (market refresh stability, social preview, and receipt polaroid clipping fixes: the SDK accepts `alpha_vector` or `state_vector` on list and single-market responses so one renamed or incomplete live row cannot blank Discover, Profile, Leaderboard, The Wire, or receipt refreshes. The desktop grid reserves a fixed 380px artifact column, mobile receipts size the SVG to the safe viewport width, SVG caption footer/date baselines are fixed and text compresses instead of ellipsizing, comparison polaroids compute safe two-up widths, Polaroid SVG defs are unique per rendered instance, the main receipt and user comparison Polaroids receive the same bimodal `secondPeak` plus full `{x,y}` consensus curve, the crowd Polaroid draws from the exact market consensus curve instead of a Gaussian approximation, and X/Twitter crawlers get a PNG card via `og-card.png`. Conviction suite is now 500 tests across 37 files, all passing including live smoke.) Update this file whenever behavior changes.*
