...
*Last updated: 2026-05-18 (receipt polaroid sizing fixes: the desktop grid reserves a fixed 380px artifact column with `minmax(0, 1fr) minmax(0, 380px)`, while mobile receipts now size the SVG to the safe viewport width, capped at 380px, instead of staying at 280px. Chromium verification at 390x844 on `/r/60/m60_pos_2208?fresh=1` confirms a 358 x 537 SVG, a square 326 x 326 photo, and visible footer/date text. Production Vercel is rebuilt through `https://conviction-receipts.vercel.app/`. Conviction suite is now 494 tests across 37 files: 489 deterministic tests pass with the 5 live smoke tests skipped.) Update this file whenever behavior changes.*
