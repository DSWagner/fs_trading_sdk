...
*Last updated: 2026-05-11 (overflow & chart-frame fixes: BetFlow no longer produces a horizontal scrollbar at any browser zoom level — the grid template column on mobile is now `minmax(0, 1fr)` instead of bare `1fr` (bare `1fr` resolves to `minmax(min-content, 1fr)`, which the Recharts ResponsiveContainer was expanding to ~1500px); all Polaroid SVGs carry `max-width: 100%` so they can't escape their grid track; the BetFlow outer wrapper has `overflow-x: clip` for belt-and-braces; the consensus chart legend now sits inside the bordered card on every viewport (the previous `bottom: -10px` offset pushed it past the rounded bottom edge); empirical verification (`scripts/verify-conviction/verify-overflow.mjs`) probes 9 viewports from 1440x900 down to 360x800 and asserts `document.scrollWidth <= clientWidth` on every one, plus legend bottom <= card bottom; full Vitest suite (1053 tests) and the existing `verify-step4-pair.mjs` and `verify-rarity.mjs` browser checks all pass with zero regressions). Update this file whenever behavior changes.*
