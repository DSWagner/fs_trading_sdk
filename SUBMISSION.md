...
> | ✓ done | 500 conviction tests pass, including 5 live smoke tests + vite + docusaurus builds clean. |
...
Run from the repo root with `npx vitest run tests/conviction`. The live tests are free and hit the dev engine; they pass after the SDK's market mappers were updated to accept `alpha_vector` or `state_vector` from live responses:

- **500 Conviction-specific tests** across 37 files (60+ tests added in the late-evening 2026-05-13 work for the new replay sparkline, comparison pair, achievements engine + strip, ErrorBoundary, and unified ShareKit; 20 tests added 2026-05-14 midday for the `predictionLabel` prop on the Polaroid scale strip, the ComparisonPair crowd-polaroid render, the Profile section ordering, and the relocated Receipt share panel; **75 tests added 2026-05-14 afternoon for the four flagship features: streak halo math + render, Receipt-for-Receipt challenge plumbing + button visibility, calibration score + leaderboard aggregation, and the Ed25519 sign / verify / tamper-detect round-trip + verify-badge render**; 10 tests added on 2026-05-18 for the desktop Receipt grid overflow, mobile receipt width, caption footer visibility, duplicate SVG-def isolation, matching main/comparison Polaroid silhouettes, exact consensus-curve crowd Polaroids, and comparison-pair no-clip regressions; an early-Convex-Hull-Frontier widget plus its 14 tests were removed on 2026-05-17 after user feedback that the visualization did not communicate clearly enough to keep):
  ...
  - `polaroid-render.test.tsx` (69 tests): SVG render under empty reasoning, 1 KB reasoning, 200-char title, prediction at and outside bounds, every shape, every resolution state, six widths, deterministic rendering, duplicate SVG-def IDs across repeated same-receipt renders, scale strip with bounds + prediction + outcome values, sentence-style footer, regression test for the empty-filter bug, animation phase progression (pre to running to done) with fake timers, unmount cleanup, mid-animation tear-down, end-to-end resolved-bet content verification.
  ...
  - `comparison-pair-render.test.tsx` (7 tests): user/crowd labels, safe two-up columns, loading/degenerate states, exact match between the main receipt Polaroid and the user comparison Polaroid for bimodal calls, and exact consensus-curve rendering for the crowd Polaroid.
...
- **812 SDK tests** still pass (the 3 tests in `tests/client-auth.test.ts` require a localhost:8000 server with password-based auth, which the competition does not use, and `tests/api-integration.test.ts` requires `FS_TEST_USERNAME` / `FS_TEST_PASSWORD` env vars that are also not part of this submission).
...
The Conviction-only suite is the recommended verification path: `npx vitest run tests/conviction` runs to completion offline and is the suite that documents the project's invariants.
