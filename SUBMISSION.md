...
> | ✓ done | 492 deterministic conviction tests pass, 5 live smoke tests skipped + vite + docusaurus builds clean. |
...
Run from the repo root with `CONVICTION_SKIP_LIVE=1 npx vitest run tests/conviction` for deterministic local verification. The uns skipped live tests are free and hit the dev engine, but currently depend on a live market-list response that is missing `alpha_vector` for at least one item:

- **497 Conviction-specific tests** across 37 files (60+ tests added in the late-evening 2026-05-13 work for the new replay sparkline, comparison pair, achievements engine + strip, ErrorBoundary, and unified ShareKit; 20 tests added 2026-05-14 midday for the `predictionLabel` prop on the Polaroid scale strip, the ComparisonPair crowd-polaroid render, the Profile section ordering, and the relocated Receipt share panel; **75 tests added 2026-05-14 afternoon for the four flagship features: streak halo math + render, Receipt-for-Receipt challenge plumbing + button visibility, calibration score + leaderboard aggregation, and the Ed25519 sign / verify / tamper-detect round-trip + verify-badge render**; 7 tests added on 2026-05-18 for the desktop Receipt grid overflow, mobile receipt width, caption footer visibility, and comparison-pair no-clip regressions; an early-Convex-Hull-Frontier widget plus its 14 tests were removed on 2026-05-17 after user feedback that the visualization did not communicate clearly enough to keep):
...
*Last updated: 2026-05-18: receipt polaroid clipping fixes. The Receipt page now reserves a fixed desktop polaroid column, uses the safe viewport width on mobile, keeps the SVG footer/date directly under the title, and shrinks comparison polaroids to fit the available two-up spread. The Conviction suite is **497 tests across 37 files**; `CONVICTION_SKIP_LIVE=1` gives 492 passing deterministic tests and 5 skipped live smoke tests, with regressions for the sizing and no-clip contracts.*
